...
function createShop() {
    ...
    <div id="shop">
        ...
        <div id="shopTooltipWeapon"></div>
        <div id="shopTooltipUpgrade"></div>
    </div>`;
...
function positionTooltipWeapon(card) {
    const tooltip = document.getElementById("shopTooltipWeapon");
...
function positionTooltipUpgrade(card) {
    const tooltip = document.getElementById("shopTooltipUpgrade");
    const shopHeight = document.getElementById("shop").offsetHeight;
    const cardMidY = card.offsetTop + card.offsetHeight / 2;
...
function showWeaponTooltip(slot) {
    const weaponNum = slot === 1 ? weaponNumber1 : weaponNumber2;
    const newWeapon = weapons[weaponNum];
    ...
    let bars = `<p class="ttTitle">VS CURRENT</p>`;
    for (const s of stats) {
        const total = s.cur + s.val || 1;
        const newColor = s.val > s.cur ? "#7a9050" : s.val < s.cur ? "#8a4545" : "#aab09a";
        const curColor = s.cur > s.val ? "#7a9050" : s.cur < s.val ? "#8a4545" : "#aab09a";
        bars += `
    <div class="ttRow">
        <span class="ttLabel">${s.label}</span>
        <div class="ttBars">
            <div class="ttBar ttCurrent" style="flex:${s.cur / total};background:${curColor}"></div>
            <div class="ttBar ttNew"     style="flex:${s.val / total};background:${newColor}"></div>
        </div>
    </div>`;

        const html = `
        <div id="ttStatsPanel">${bars}</div>
        <div id="ttWeaponImg">
            <img src="./img/sprites/Guns/${weaponNum}.png" alt="${newWeapon.name}">
        </div>`;

        const tooltip = document.getElementById("shopTooltipWeapon");
        tooltip.innerHTML = html;
        positionTooltipWeapon(document.getElementById("shopWeapon" + slot));
    }
}
...
function showUpgradeTooltip(slot) {
    ...
    const tooltip = document.getElementById("shopTooltipUpgrade");
    tooltip.innerHTML = html;
    positionTooltipUpgrade(document.getElementById("shopUpgrade" + slot));
}

function hideShopTooltip() {
    const tooltip = document.getElementById("shopTooltipWeapon");
    if (tooltip) tooltip.style.display = "none";
    const tooltip2 = document.getElementById("shopTooltipUpgrade");
